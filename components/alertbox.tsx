	...
	return (
		<LabeledAlertbox header="Welcome!">
			<h3 className="highlight">
				(Notifications won't be enabled until the next step. Please don't leave
				yet!)
			</h3>
			<p>
				You've made your first filter! Your filters are saved to your account
				and can be synced across devices using your{" "}
				<span className="highlight">unique user ID</span>, shown below.
			</p>
			...
			<p style={{ marginTop: "0" }}>
				...
				<br />
				<i>(Limited to alphanumeric characters, dashes, and spaces!)</i>
			</p>
...
export function NotificationAlertbox(props: NotificationAlertboxProps) {
	...
	return (
		<LabeledAlertbox header="Enable Notifications">
			<p>
				SplatNet Alerts needs notifications enabled to work properly! When
				enabled, a push notification will be sent to this device whenever
				matching gear appears in the SplatNet shop.
			</p>
			<p>Notifications can be switched on or off at any time.</p>
			<div className="gap" style={{ alignContent: "center" }}>
				...
				>
					<Image
						...
						layout="responsive"
						alt="Notification settings box, on Chrome. The dialog box says, 'splatnet.netlify.app is asking you to show notifications.' The dropdown is set to 'Remember my decision: Forever'."
					/>
				</div>
				<div
					className="hdiv gap"
				...
				>
					...
					<p style={{ margin: "0", color: "var(--highlight)" }}>
						<b>
							When prompted, set notifications to 'Forever' or else you may stop
							getting notified!
						</b>
					</p>
				</div>
			...
			<div className={styles.twoButtonContainer}>
				...
				>
					...
					<div className="hdiv gap">
						<span
							className="material-symbols-rounded md-24 md-light"
...
export function IOSIncompatibleVersionAlertbox(props: {
		...
		>
			<p>
				<span className="highlight">
					SplatNet Alerts uses the web push API to send notifications, which is
					only supported in iOS 16.4 or above.
				</span>
				{version[0] !== -1 ? (
					<>
						<br />
						...
						<span className="highlight">{versionText}</span>
						<br />
						<br />
						You'll need to either update your iOS version or use a different
						device to receive alerts. The rest of the website will still work on
						your device, just without notifications.
						<br />
						<br />
						You can read more about this on the{" "}
						{makeLink(
							"project's GitHub page",
							"https://github.com/ShrimpCryptid/SplatNet-Alerts/issues/2"
						)}
						.
					</>
				) : (
					<>
						<br />
						<br />
						It looks like you might be using an incompatible iOS device, but the
						version can't be detected so I don't know for sure. Please check
						your iOS version, then come back if there aren't any issues.
						<br />
						<br />
						If you think you've incorrectly received this message, please{" "}
						{makeLink(
							"report it on GitHub",
							"https://github.com/ShrimpCryptid/SplatNet-Alerts/issues"
						)}{" "}
						so I can issue a fix!
					</>
				)}
			</p>
		</LabeledAlertbox>
